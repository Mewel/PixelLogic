...
public class PixelLogicAssets {
    ...
    protected void loadFonts() {
        int base = (int) (GAME_FONT_BASE * Gdx.graphics.getDensity() * getDistanceFactor());
        int textSize = Math.max(GAME_FONT_MIN, base);
        int h2Size = Math.max(textSize + GAME_FONT_BASE, (int) (Math.ceil(textSize * 1.1f / GAME_FONT_BASE) * GAME_FONT_BASE));
        int h1Size = Math.max(textSize + GAME_FONT_BASE * 2, (int) (Math.ceil(textSize * 1.4f / GAME_FONT_BASE) * GAME_FONT_BASE));
    ...
    private void loadMainFont() {
        ...
        gameFont.fontParameters.color = Color.WHITE;
        int calculatedSize = (int) (Gdx.graphics.getHeight() * 0.03f);
        gameFont.fontParameters.size = Math.max(16, calculatedSize);
    ...
    private List<FileHandle> getLevelHandles() {
        ...
        FileHandle assetsTxt = Gdx.files.internal("assets.txt");
        if (assetsTxt.exists()) {
            String[] lines = assetsTxt.readString().split("\\r?\\n");
            for (String line : lines) {
                line = line.trim();
                if (line.startsWith(LEVEL_DIRECTORY + "/") && line.endsWith("/collection.json")) {
                    String folderPath = line.substring(0, line.lastIndexOf("/collection.json"));
                    handles.add(Gdx.files.internal(folderPath));
                }
            }
        } else {
            FileHandle levelFolder = Gdx.files.internal(LEVEL_DIRECTORY);
            handles.addAll(getLevelHandlesFallback(levelFolder.list()));
        }
        return handles;
    }

    private List<FileHandle> getLevelHandlesFallback(FileHandle[] collectionsHandle) {
        List<FileHandle> handles = new ArrayList<>();
        for (FileHandle collectionHandle : collectionsHandle) {
            ...
            handles.addAll(getLevelHandlesFallback(collectionHandle.list()));
        }
        return handles;
    }
