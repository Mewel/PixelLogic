...
public class PixelLogicUIUtil {
    ...
    public static int getBaseHeight() {
        // Ideal size: ~8% of the screen height (gentler scaling for tall phones)
        float idealSize = Gdx.graphics.getHeight() * 0.08f;
        // Divide by 24 and round to find the nearest perfect integer multiplier (1x, 2x, 3x...)
        int scale = Math.max(1, Math.round(idealSize / BASE_SIZE));
        // Multiply back by 24 to guarantee crisp pixel art!
        return scale * BASE_SIZE;
    }

    public static int getIconBaseHeight() {
        float idealSize = Gdx.graphics.getHeight() * 0.04f;
        int scale = Math.max(1, Math.round(idealSize / BASE_SIZE));
        return scale * BASE_SIZE;
    }
